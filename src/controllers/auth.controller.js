...
export const signup = async (req, res, next) =>{
  ...
  try {
    ...
    if (!validatedData.success) {
      logger.error('Validation error during signup:', formatValidationErrors(validatedData.error));  
    ...
    cookies.setCookie(res, 'token', token, { httpOnly: true, secure: process.env.NODE_ENV === 'production', sameSite: 'strict', maxAge: 24 * 60 * 60 * 1000 });
...
export const signOut = (req, res) => {
  try{cookies.clearCookie(res, 'token');
